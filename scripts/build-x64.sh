...
# 构建脚本 (x86_64) - 构建后端和前端，自动生成 OTA 包
...
print_windows_notice() {
    if is_windows_bash; then
        ...
        echo "   完整 OTA 后端需要 Linux x86_64 musl 工具链。"
        echo "   推荐在 WSL2 Ubuntu 中运行: ./scripts/build-x64.sh --no-upx"
        echo ""
    fi
}
...
for arg in "$@"; do
    case $arg in
        ...
        --no-ota)
            ...
            ;;
        --help|-h)
            echo "用法: ./scripts/build-x64.sh [选项]"
            echo ""
            echo "选项:"
            ...
            echo ""
            echo "示例:"
            echo "  ./scripts/build-x64.sh                    # 构建 + UPX + OTA 包"
            echo "  ./scripts/build-x64.sh --no-upx           # 不压缩"
            echo "  ./scripts/build-x64.sh --no-ota           # 不生成 OTA 包"
            echo "  ./scripts/build-x64.sh --frontend-only    # 仅构建前端"
            exit 0
            ;;
    esac
done
...
if [ "$BUILD_BACKEND" = true ]; then
    ...
    # 对于 x86_64-unknown-linux-musl，在 x86_64 本机上可以使用 musl-gcc 编译
    # 非 x86_64 主机（如 ARM）需要交叉编译器
    HOST_ARCH="$(uname -m)"
    if [ "$HOST_ARCH" = "x86_64" ] || [ "$HOST_ARCH" = "amd64" ]; then
        ...
        # 本机构建 x86_64 musl，直接用 musl-gcc
        export CC_x86_64_unknown_linux_musl=musl-gcc
        export CARGO_TARGET_X86_64_UNKNOWN_LINUX_MUSL_LINKER=musl-gcc
    else
        # 交叉编译：非 x86_64 主机需要 x86_64 musl 交叉编译器
        if ! command -v x86_64-unknown-linux-musl-gcc &> /dev/null; then
            echo "❌ 错误: 未找到 x86_64-unknown-linux-musl-gcc"
            echo ""
            echo "当前主机架构: $HOST_ARCH"
            echo "需要安装 x86_64 Linux musl 交叉编译工具链。"
            echo ""
            echo "macOS 可参考:"
            echo "  brew tap messense/macos-cross-toolchains"
            echo "  brew install x86_64-unknown-linux-musl"
            exit 1
        fi
...
EOF
        ...
        # 打包
        OTA_FILE="release/simadmin-x64_${VERSION}.tar.gz"
        echo "打包 OTA..."
        cd "$OTA_TMP"
...
echo ""
echo "=========================================="
echo "部署命令: ./scripts/deploy-x64.sh"
echo "=========================================="
