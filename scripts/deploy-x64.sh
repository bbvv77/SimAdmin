...
# 部署脚本 (x86_64) - 通过 ADB 将构建产物部署到目标设备
...
for arg in "$@"; do
    case $arg in
        ...
        --target=*)
            ...
            ;;
        --help|-h)
            echo "用法: ./scripts/deploy-x64.sh [选项]"
            echo ""
            echo "选项:"
            ...
            echo ""
            echo "示例:"
            echo "  ./scripts/deploy-x64.sh                      # 部署前端和后端"
            echo "  ./scripts/deploy-x64.sh --backend-only       # 只部署后端"
            echo "  ./scripts/deploy-x64.sh --frontend-only      # 只部署前端"
            echo "  ./scripts/deploy-x64.sh --no-restart         # 部署但不停止服务"
            echo "  ./scripts/deploy-x64.sh --target=/data/app   # 部署到指定路径"
            exit 0
            ;;
        *)
            if [[ ! "$arg" =~ ^-- ]]; then
...
BACKEND_BIN="backend/target/x86_64-unknown-linux-musl/release/simadmin"
FRONTEND_DIR="frontend/dist"

echo "🚀 通过 ADB 部署到 ${TARGET_PATH} (x86_64)"
echo ""
...
if [ "$DEPLOY_BACKEND" = true ]; then
    if [ ! -f "$BACKEND_BIN" ]; then
        echo "❌ 后端二进制不存在，请先运行 ./scripts/build-x64.sh"
        exit 1
    fi
fi
...
if [ "$DEPLOY_FRONTEND" = true ]; then
    if [ ! -d "$FRONTEND_DIR" ]; then
        echo "❌ 前端构建产物不存在，请先运行 ./scripts/build-x64.sh --frontend-only"
        exit 1
    fi
fi
