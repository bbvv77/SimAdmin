...
# 打包 OTA 更新包 (x86_64)
# 输出: release/simadmin-x64_{version}.tar.gz
...
# 检查构建产物
BINARY_PATH="backend/target/x86_64-unknown-linux-musl/release/simadmin"
FRONTEND_DIR="frontend/dist"

if [ ! -f "$BINARY_PATH" ]; then
    echo "❌ 错误: 后端二进制不存在: $BINARY_PATH"
    echo "请先运行: ./scripts/build-x64.sh"
    exit 1
fi

if [ ! -d "$FRONTEND_DIR" ]; then
    echo "❌ 错误: 前端构建产物不存在: $FRONTEND_DIR"
    echo "请先运行: ./scripts/build-x64.sh --frontend-only"
    exit 1
fi
